...
the full plate is needed for this visualization, the functions will only work with 
...
This displays the minimal ampount of code required to go from raw data file to 
antibody binding positivity call.
<<quick-analysis, cache=TRUE, results='hide'>>=
mapFile <- system.file("extdata/mapping.csv", package = "PEP.db")
dirToParse <- system.file("extdata/gpr_samples", package = "PEP.db")
ps <- makePeptideSet(files = NULL, path = dirToParse,
                       mapping.file = mapFile, log = TRUE,
                       rm.control.list = c("JPT-control", "Ig", "Cy3"),
                       empty.control.list= c("empty", "blank control"))
data(pep_hxb2)
...
V_calls <- makeCalls(ps, freq = TRUE, group = "treatment", 
                     cutoff = .1, method = "FDR", verbose = TRUE)
@
