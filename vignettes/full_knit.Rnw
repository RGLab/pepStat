...
\section{Results}
\texttt{pepStat} extends the visualisation package \texttt{Gviz} and adapts it 
for peptide and protein sequences.

The most simple plots can display an overview of the difference between groups. 
In this case, comparing placebo and vaccine.
<<plot-inter, fig.height=4>>=
plot_inter(psmSet, V_calls)
@
