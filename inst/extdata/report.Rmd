...
anno <- ATrack(start=start(features), end=end(features), name="landmarks", id=getName(features), showFeatureId=TRUE, fill=1:nrow(features))
pro_anno <- ATrack(start=start(proteins),end=end(proteins), id=proteins$name, name="Proteins",size=1.5, showFeatureId=TRUE, stacking="dense", fill=1:nrow(proteins))
dt <- DTrack(start=position(pnSet[names(freq),]), width=0, data=freq, name="Frequency", type="l")
```

## Plotting
```{r plotTracks, fig.align='center', fig.height=8, fig.width=12}
plotTracks(c(pat, anno, pro_anno, dt), from=from, to=to)
```
