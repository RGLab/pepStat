...
It returns a new object of class \texttt{peptideSet}. It contains only the sequence of the peptides and the intensities measured. Any additional information must be passed using a known collection of peptides. In this example, we use pep\_hxb2 available in the \texttt{PEP.db} package.
