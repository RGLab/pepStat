...
bwplot(Intensity ~ ptid, groups=visit, panel = panel.superpose, panel.groups=panel.bwplot, alpha=.5, cex=.3, fill=palette, pch="|", data=y.long.raw[y.long.raw$ptid%in%u.ptid[1:min(16,length(u.ptid))],])
@
...
bwplot(Intensity ~ ptid, groups=visit, panel = panel.superpose, panel.groups=panel.bwplot, alpha=.5, cex=.3, fill=palette, pch="|", data=y.long[y.long$ptid%in%u.ptid[1:min(16,length(u.ptid))],])
@
